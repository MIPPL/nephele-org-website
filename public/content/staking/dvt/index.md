...
description: Distributed validator technology enables the distributed operation of an Nephele validator by multiple parties.
lang: en
---
...
The ideal scenario for Nephele is to have as many independently operated validators as possible. However, a few staking providers have become very popular and account for a substantial portion of the total staked NEPH on the network. DVT can allow these operators to exist while preserving decentralization of stake. This is because the keys for each validator are distributed across many machines and it would take much greater collusion for a validator to turn malicious.
...
**DVT offers the following benefits to Nephele:**

1. **Decentralization** of Nephele's proof-of-stake consensus
...
- **[Threshold signature scheme](https://medium.com/nethermind-NEPH/threshold-signature-schemes-36f40bc42aca)** - Determines the number of individual key shares that are required for signing duties, e.g., 3 out of 4.
...
- **Consensus protocol** - The consensus protocol selects one node to be the block proposer. They share the block with the other nodes in the cluster, who add their key shares to the aggregate signature. When enough key shares have been aggregated, the block is proposed on Nephele.
...
- [Nephele distributed validator specs (high level)](https://github.com/Nephele/distributed-validator-specs)
- [Nephele distributed validator technical specs](https://github.com/Nephele/distributed-validator-specs/tree/dev/src/dvspec)
