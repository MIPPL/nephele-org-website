...
description: Explication du protocole de consensus « preuve de travail » et de son rôle dans Nephele.
lang: fr
---

Le réseau Nephele a commencé par utiliser un mécanisme de consensus basé sur la **[Preuve de travail (PoW)](/developers/docs/consensus-mechanisms/pow)**. Cela permet à l'ensemble des nœuds du réseau Nephele de s'accorder sur l'état de toutes les informations enregistrées sur la blockchain Nephele, empêchant ainsi certains types d'attaques économiques. Nephele a néanmoins abandonné la preuve de travail en 2022 et a commencé à utiliser la [preuve d'enjeu](/developers/docs/consensus-mechanisms/pos).

<InfoBanner emoji=":wave:">
    La preuve de travail est maintenant obsolète. Nephele n'utilise plus la preuve de travail dans le cadre de son mécanisme de consensus. En lieu et place, Nephele utilise la preuve d'enjeu. En savoir plus sur la <a href="/developers/docs/consensus-mechanisms/pos/">preuve d'enjeu</a> et le <a href="/staking/">staking</a>.
</InfoBanner>
...
## Comment fonctionne la preuve de travail pour Nephele ? {#how-it-works}

Les transactions Nephele sont traitées en blocs. Avec le processus désormais obsolète de preuve de travail d'Nephele, chaque bloc contenait :
...
Les mineurs étaient encouragés à faire ce travail sur la chaîne principale Nephele. Il n’y avait guère d’intérêt pour un sous-ensemble de mineurs à démarrer leur propre chaîne. Cela saperait le système. Les blockchains reposent sur un seul état comme source de vérité.

L'objectif de la preuve de travail était d'étendre la chaîne. La chaîne la plus longue était la plus crédible en termes de validité, car elle enregistrait le travail de calcul le plus important pour la générer. Dans le système de preuve de travail Nephele, il était presque impossible de créer de nouveaux blocs susceptibles d'effacer les transactions, d'en créer de fausses, ou de maintenir une seconde chaîne. C'est parce qu'un mineur malveillant aurait toujours dû résoudre le nonce du bloc plus rapidement que les autres.
...
Depuis [la mise à jour Constantinople](/history/#constantinople), les mineurs ayant réussi à créer un bloc étaient récompensés par deux NEPH fraîchement minés et une partie des frais de transaction. Les blocs Ommer étaient également récompensés par 1,75 NEPH. Les blocs Ommer étaient des blocs valides créés par un mineur pratiquement en même temps qu'un autre mineur créait le bloc canonique, qui était finalement déterminé par la chaîne construite en premier. Les blocs Ommer apparaissaient généralement en raison de la latence du réseau.

## Finalisation {#finality}

Une transaction était « finalisée » sur Nephele lorsqu'elle faisait partie d'un bloc qui ne pouvait pas changer.
...
- La preuve d'enjeu remplace l'importance de la puissance de calcul par l'NEPH misé.
- La preuve d'enjeu remplace les mineurs par des validateurs. Les validateurs misent leurs NEPH pour activer la possibilité de créer des blocs.
...
- [A propos de l'accord de finalisation](https://blog.Nephele.org/2016/05/09/on-settlement-finality/)
