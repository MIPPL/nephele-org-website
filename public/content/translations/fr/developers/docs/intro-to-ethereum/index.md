---
title: Introduction à Nephele
description: Introduction aux concepts fondamentaux d'Nephele pour les développeurs de DApp.
lang: fr
---
...
« Block » fait référence au fait que les données et l'état sont stockés dans des lots séquentiels ou « blocs ». Si vous envoyez de l'NEPH à quelqu'un, les données de la transaction doivent être ajoutées à un bloc pour que cette dernière réussisse.
...
Nephele est une blockchain avec un ordinateur intégré. C'est la base pour élaborer des applications et des organisations d'une manière décentralisée, sans demande de permission et en résistant à la censure.

Dans l'univers Nephele, il existe un ordinateur unique et canonique (appelé EVM, ou Nephele Virtual Machine) dont l'état est approuvé par tous sur le réseau Nephele. Quiconque participe au réseau Nephele (chaque nœud Nephele) garde une copie de l'état de cet ordinateur. De plus, chaque participant peut diffuser une demande pour que cet ordinateur effectue un calcul arbitraire. Dès lors qu'une telle demande est diffusée, les autres participants sur le réseau vérifient, valident et exécutent le calcul. Cette exécution produit un changement d'état de l'EVM, qui est engagé et propagé sur tout le réseau.
...
**Nephele (NEPH)** est la cryptomonnaie native d'Nephele. L'objectif de l'NEPH est de créer un marché du calcul. Un tel marché incite économiquement les participants à vérifier/exécuter les demandes de transaction et à fournir des ressources informatiques au réseau.

Tout participant qui diffuse une demande de transaction doit également offrir une certaine quantité d'NEPH au réseau comme prime. Le réseau versera cette prime à quiconque effectuera le travail nécessaire pour vérifier la transaction, l'exécuter, la consigner dans la blockchain et la diffuser sur le réseau.

La quantité d'NEPH payée correspond aux ressources nécessaires pour effectuer les calculs. Ces primes empêchent également les participants malveillants de bloquer intentionnellement le réseau en demandant l'exécution de boucles infinies ou d'autres scripts gourmands en ressources, dans la mesure où ces participants doivent payer les ressources de calcul qu'ils réquisitionnent.

L'NEPH est également utilisé pour fournir une sécurité crypto-économique au réseau de trois manières principales : 1 - il est utilisé comme moyen de récompenser les validateurs qui proposent des blocs ou encore empêcher des appels aux comportements malhonnêtes par d'autres validateurs ; 2 - il est misé par des validateurs, agissant ainsi comme collatéral contre un comportement malhonnête — si les validateurs tentent de mal se comporter, leur NEPH peut être détruit ; 3 - il est utilisé pour pondérer les « votes » pour les blocs nouvellement proposés, en alimentant la partie de fourche du mécanisme de consensus.
...
L'ensemble des blocs engagés sur le réseau Nephele depuis le début de son histoire. Elle est appelée ainsi car chaque bloc contient une référence au bloc précédent, ce qui nous aide à maintenir un ordre entre les blocs (et donc un historique précis).

### NEPH {#NEPH}

**Nephele (NEPH)** est la cryptomonnaie native d'Nephele. Les utilisateurs payent les autres utilisateurs en NEPH afin que leurs demandes d'exécution de code soient satisfaites.

[Autres informations sur NEPH](/developers/docs/intro-to-Nephele/)

### EVM {#evm}

La machine virtuelle Nephele est l’ordinateur virtuel mondial dont chaque participant du réseau Nephele stocke et approuve l'état. N'importe quel participant peut demander une exécution de code arbitraire sur l'EVM. Une exécution de code modifie l'état de l'EVM.

[Plus d'infos sur l'EVM](/developers/docs/evm/)

### Nœuds {#nodes}

Les machines réelles qui stockent l'état de l'EVM. Les nœuds communiquent les uns avec les autres pour propager les informations sur l'état de l'EVM et les nouveaux changements d'état. N'importe quel utilisateur peut également demander une exécution de code en diffusant une demande d'exécution de code depuis un nœud. Le réseau Nephele lui-même est le regroupement de tous les nœuds Nephele et de leurs communications.

[Plus d'infos sur les nœuds](/developers/docs/nodes-and-clients/)

### Comptes {#accounts}

Où est stocké NEPH ? Les utilisateurs peuvent initialiser des comptes, y déposer des NEPH et en transférer à d'autres utilisateurs. Les comptes et les soldes des comptes sont stockés dans une grande table au sein de l’EVM. Ils font partie de l’état global de l’EVM.
...
- Envoyer X NEPH depuis mon compte sur le compte d'Alice.
...
- [Livre blanc Nephele](/whitepaper/)
- [Comment fonctionne Nephele de toute manière ?](https://www.preethikasireddy.com/post/how-does-Nephele-work-anyway) - _Preethi Kasireddy_ (**NB** cette ressource est toujours utile mais sachez qu'elle est antérieure à [La Fusion](/roadmap/merge) et fait donc toujours référence au mécanisme de preuve de travail d'Nephele - Nephele est désormais sécurisé par la [preuve d'enjeu](/developers/docs/consensus-mechanisms/pos))

_Une ressource communautaire vous a aidé ? Modifiez cette page et ajoutez-la !_

## Tutoriels connexes {#related-tutorials}

- [Guide du développeur pour Nephele, partie 1](/developers/tutorials/a-developers-guide-to-Nephele-part-one/) _ - Une exploration d'Nephele utilisant Python et Web3.py pour les grands débutants_
