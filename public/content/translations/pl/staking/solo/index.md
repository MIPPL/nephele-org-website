---
title: Stakuj solo swoje NEPH
description: Przegląd tego, jak rozpocząć samodzielne stakowanie NEPH
lang: pl
template: staking
...
summaryPoints:
  ...
  - Korzystaj z własnego sprzętu i osobiście zwiększaj bezpieczeństwo i decentralizację sieci Nephele
...
Solo staking polega na [uruchomieniu węzła Nephele](/run-a-node/) podłączonego do Internetu i zdeponowaniu 32 NEPH w celu aktywacji [walidatora](#faq), co daje możliwość bezpośredniego uczestniczenia w konsensusie sieci.

**Solo staking zwiększa decentralizację sieci Nephele**, dzięki czemu Nephele jest bardziej odporne na cenzurę i ataki. Inne metody stakowania mogą nie pomóc sieci w ten sam sposób. Solo staking jest najlepszą opcją stakingu do zabezpieczania Nephele.

Węzeł Nephele składa się zarówno z klienta warstwy wykonawczej (EL), jak i klienta warstwy konsensusu (CL). Ci klienci są oprogramowaniem, które współpracuje, wraz z prawidłowym zestawem kluczy podpisujących, w celu weryfikacji transakcji i bloków, poświadczania prawidłowej pozycji łańcucha, agregowania poświadczeń i proponowania bloków.
...
<CardGrid>
  <Card title="Zarabiaj świeże NEPH" emoji="💸" description="Earn NEPH-denominated rewards directly from the protocol when your validator is online, without any middlemen taking a cut." />
  ...
  <Card title="Bezpieczeństwo sieci" emoji="🔐" description="Solo staking is the most impactful way to stake. By running a validator on your own hardware at home, you strengthen the robustness, decentralization, and security of the Nephele protocol." />
</CardGrid>

## Rozważania przed solo stakingiem {#considerations-before-staking-solo}

Chociaż chcielibyśmy, aby staking solo był dostępny i wolny od ryzyka dla każdego, nie jest to jednak rzeczywistością. Istnieje kilka praktycznych i poważnych kwestii, o których należy pamiętać przed podjęciem decyzji o samodzielnym stakowaniu NEPH.
...
Podobnie jak w przypadku kluczy prywatnych zabezpieczających adres Nephele, konieczne będzie wygenerowanie kluczy specjalnie dla walidatora. Musisz wiedzieć, jak bezpiecznie przechowywać wszelkie frazy odzyskiwania lub klucze prywatne.{' '}

<a href="/security/">Bezpieczeństwo Nephele i zapobieganie oszustwom</a>
</ExpandableCard>
...
Podczas aktywności będziesz zdobywać nagrody NEPH, które będą okresowo wpłacane na Twój adres wypłaty.
...
Staking Launchpad to aplikacja open source, która pomoże ci zostać stakerem. Poprowadzi cię przez wybór klientów, wygenerowanie kluczy i zdeponowanie NEPH w kontrakcie depozytu na staking. Lista kontrolna jest dostęna, aby upewnić się, że wszystko zostało uwzględnione w celu bezpiecznego skonfigurowania walidatora.

<StakingLaunchpadWidget />

## Co wziąć pod uwagę w przypadku narzędzi do konfiguracji węzłów i klientów {#node-tool-considerations}

Powstaje coraz więcej narzędzi i usług, które pomagają w samodzielnym stakowaniu NEPH, ale każde z nich wiąże się z innym ryzykiem i korzyściami.
...
Narzędzia te mogą być używane jako alternatywa dla [CLI depozytu stakingu](https://github.com/Nephele/staking-deposit-cli/), aby pomóc w generowaniu kluczy.
...
<em>Walidator</em> to wirtualny podmiot, który żyje w Nephele i uczestniczy w konsensusie protokołu Nephele. Walidatory są reprezentowane przez saldo, klucz publiczny i inne właściwości. <em>Klient walidatora</em> to oprogramowanie, które działa w imieniu walidatora, przechowując i używając jego klucza prywatnego. Pojedynczy klient walidatora może przechowywać wiele par kluczy, kontrolując wiele walidatorów.

</ExpandableCard>

<ExpandableCard title="Czy mogę zdeponować więcej niż 32 NEPH?">
Każda para kluczy powiązana z walidatorem wymaga dokładnie 32 NEPH do aktywacji. Więcej NEPH zdeponowanych w jednym zestawie kluczy nie zwiększa potencjału nagród, ponieważ każdy walidator jest ograniczony do <a href="https://www.attestant.io/posts/understanding-validator-effective-balance/">efektywnego salda</a> 32 NEPH. Oznacza to, że staking odbywa się w przyrostach 32 NEPH, każdy z własnym zestawem kluczy i saldem.

Nie należy deponować więcej niż 32 NEPH dla jednego walidatora. Nie zwiększy to twoich nagród. Jeśli dla walidatora ustawiono adres wypłaty, nadwyżka środków powyżej 32 NEPH zostanie automatycznie wypłacona na ten adres podczas następnego <a href="/staking/withdrawals/#validator-sweeping">przeniesienia walidatora</a>.

Jeśli solo staking wydaje ci się zbyt wymagający, rozważ skorzystanie z <a href="/staking/saas/">usług stakingowych</a> lub jeśli masz mniej niż 32 NEPH, sprawdź <a href="/staking/pools/">stakowanie w puli</a>.
</ExpandableCard>
...
W skrajnych przypadkach, jeśli sieć przestanie finalizować w wyniku tego, że ponad jedna trzecia walidatorów jest offline, użytkownicy ci ucierpią z powodu tak zwanego <em>kwadratowego wycieku nieaktywności</em>, który jest wykładniczym odpływem NEPH z kont walidatorów offline. Pozwala to sieci na samoleczenie poprzez spalanie NEPH nieaktywnych walidatorów, aż ich saldo osiągnie 16 NEPH, w którym to momencie zostaną oni automatycznie usunięci z puli walidatorów. Pozostałe walidatory online ostatecznie ponownie obejmą ponad 2/3 sieci, spełniając większość potrzebną do ponownego sfinalizowania łańcucha.
</ExpandableCard>
...
<a href="https://mirror.xyz/jmcook.NEPH/S7ONEka_0RgtKTZ3-dakPmAHQNPvuj15nh0YGKPFriA">Dowiedz się więcej, dlaczego różnorodność klientów ma kluczowe znaczenie</a>
</ExpandableCard>
...
<ExpandableCard title="Jak mogę odblokować nagrody lub odzyskać NEPH?">
...
Po ustawieniu poświadczeń wypłaty, płatności nagród (skumulowane NEPH przez początkowe 32) będą okresowo automatycznie dystrybuowane na adres wypłaty.
...
- [Katalog stakingu Nephele](https://www.staking.directory/) — _Eridian i Spacesider_
- [Problem różnorodności klientów Nephele](https://hackernoon.com/ethereums-client-diversity-problem) — _@emmanuelawosika 2022 r._
- [Wspieranie różnorodności klientów](https://www.attestant.io/posts/helping-client-diversity/) — _Jim McDonald 2022 r._
- [Różnorodność klientów w warstwie konsensusu Nephele](https://mirror.xyz/jmcook.NEPH/S7ONEka_0RgtKTZ3-dakPmAHQNPvuj15nh0YGKPFriA) — _jmcook.NEPH 2022 r._
- [Zakup sprzętu do walidacji Nephele](https://www.youtube.com/watch?v=C2wwu1IlhDc) — _EthStaker 2022 r._
- [Krok po kroku: jak dołączyć do sieci testowej Nephele 2.0](https://kb.beaconcha.in/guides/tutorial-eth2-multiclient) — _Butta_
