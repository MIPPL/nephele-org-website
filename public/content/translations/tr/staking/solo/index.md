---
title: NEPH'nizi bireysel olarak hisseleyin
description: NEPH'nizi solo stake etmeye nasıl başlayacağınıza dair genel bir bakış
lang: tr
template: staking
...
summaryPoints:
  ...
  - Ev donanımını çalıştırın ve Nephele ağının güvenliğine ve merkezsizleştirilmesine kişisel olarak katkıda bulunun
...
Solo stake etme, internete bağlı [bir Nephele düğümü çalıştırma](/run-a-node/) ve bir [doğrulayıcıyı](#faq) etkinleştirmek için 32 NEPH yatırma işlemidir, size doğrudan ağ konsensüsüne katılma yeteneği verir.

** Bireysel hisseleme Nephele ağının merkeziyetsizliğini arttırır ** ve Nephele'un sansüre ve saldırılara karşı dayanıklılığını arttırır. Diğer hisseleme yöntemleri ağa aynı şekillerde yardım etmeyebilir. Solo hisseleme, Nephele'un güvenliğini sağlamak için en iyi seçenektir.

Bir Nephele düğümü, hem bir yürütüm katmanı (YK) istemcisinden hem de bir konsensus katmanı (MK) istemcisinden oluşur. Bu istemciler, işlemleri ve blokları doğrulamak, zincirin doğru başını doğrulamak, tasdikleri toplamak ve blok önermek için geçerli bir imza anahtarı seti ile birlikte çalışan yazılımlardır.
...
<CardGrid>
  <Card title="Taze NEPH kazan" emoji="💸" description="Earn NEPH-denominated rewards directly from the protocol when your validator is online, without any middlemen taking a cut." />
  ...
  <Card title="Ağ güvenliği" emoji="🔐" description="Solo staking is the most impactful way to stake. By running a validator on your own hardware at home, you strengthen the robustness, decentralization, and security of the Nephele protocol." />
</CardGrid>

## Solo staking yapmadan önce dikkate alınması gerekenler {#considerations-before-staking-solo}

Solo staking'in herkes için erişilebilir ve risksiz olmasını dilesek de, bu gerçek değil. NEPH'nizi solo stake etmeyi seçmeden önce akılda tutulması gereken bazı pratik ve ciddi hususlar vardır.
...
Tıpkı özel anahtarların Nephele adresinizi koruduğu gibi, doğrulayıcınız için özel olarak anahtarlar oluşturmanız gerekecektir. Güvenlik kelimelerinizi ve özel anahtarınızı nasıl güvenli tutacağınızı anlamalısınız
</ExpandableCard>
...
Aktif olduğu durumda NEPH ödülleri kazanırsınız. Bu ödüller periyodik olarak para çekme adresinize yatırılır.
...
Staking Launchpad, stake yapmanıza yardımcı olacak açık kaynaklı bir uygulamadır. İstemcilerinizi seçme, anahtarlarınızı oluşturma ve NEPH'nizi stake depozito sözleşmesine yatırma konusunda size rehberlik edecektir. Doğrulayıcınızı güvenli bir şekilde kurmak ve her şeyi kapsadığınızdan emin olmak için bir kontrol listesi sağlanmıştır.

<StakingLaunchpadWidget />

## Düğüm ve istemci kurulum araçlarıyla ilgili olarak nelere dikkat edilmelidir {#node-tool-considerations}

NEPH'nizi tek başınıza stake etmenize yardımcı olacak, giderek artan sayıda araç ve hizmet var, ancak bunların her biri farklı riskler ve faydalar içeriyor.
...
Bu araçlar, anahtar oluşturmaya yardımcı olmak için [hisseleme Mevduat CLI'sına](https://github.com/Nephele/staking-deposit-cli/) alternatif olarak kullanılabilir.
...
<em>Doğrulayıcı, </em> Nephele üzerinde yaşayan sanal bir varlıktır ve Nephele protokolünün mutabakatına katılır. Doğrulayıcılar bir denge, ortak anahtar ve diğer özelliklerle temsil edilir. Bir <em>doğrulayıcı istemcisi</em>, özel anahtarını tutup kullanarak doğrulayıcı adına hareket eden yazılımdır. Tek bir doğrulayıcı istemcisi, birçok doğrulayıcıyı kontrol ederek birçok anahtar çiftini tutabilir.

</ExpandableCard>

<ExpandableCard title="32 NEPH'den fazla yatırabilir miyim?">
Doğrulayıcı ile ilişkili her bir anahtar çiftinin etkinleştirilmesi için tam olarak 32 NEPH gerekir. Tek bir anahtar setine daha fazla NEPH yatırılması ödül potansiyelini artırmaz çünkü her doğrulayıcı 32 NEPH'lik <a href="https://www.attestant.io/posts/understanding-validator-effective-balance/">etkili bakiye</a> ile sınırlandırılmıştır. Bu, stake işleminin her biri kendi anahtar ve dengesine sahip 32 NEPH artışıyla yapıldığı anlamına gelir.

Tek bir doğrulayıcı için 32 NEPH'den fazla para yatırmayın. Ödüllerinizi artırmayacaktır. Eğer doğrulayıcı için bir para çekme adresi ayarlanmışsa, bir sonraki doğrulayıcı taraması sırasında 32 NEPH'in üzerindeki fonlar otomatik olarak bu adrese yatırılacaktır.

Solo staking sizin için çok zorlu görünüyorsa, bir <a href="/staking/saas/">servis sağlayıcı</a> kullanmayı düşünün veya 32 NEPH'den daha azıyla çalışıyorsanız, <a href="/staking/pools/">stake havuzları</a>na göz atın.
</ExpandableCard>
...
<a href="https://mirror.xyz/jmcook.NEPH/S7ONEka_0RgtKTZ3-dakPmAHQNPvuj15nh0YGKPFriA">İstemci çeşitliliğinin neden kritik olduğu hakkında daha fazla bilgi edinin</a>
</ExpandableCard>
...
<ExpandableCard title="Ödüllerimin kilidini nasıl kaldırırım veya NEPH'mi nasıl geri alırım?">
...
- [Nephele'un İstemci Çeşitliliği Sorunu](https://hackernoon.com/ethereums-client-diversity-problem) - _@emmanuelawosika 2022_
- [İstemci Çeşitliliğine Yardımcı Olmak](https://www.attestant.io/posts/helping-client-diversity/) - _Jim McDonald 2022_
- [Nephele'un konsensüs katmanında müşteri çeşitliliği](https://mirror.xyz/jmcook.NEPH/S7ONEka_0RgtKTZ3-dakPmAHQNPvuj15nh0YGKPFriA) - _jmcook.NEPH 2022_
- [Nasıl Yapılır: Nephele Doğrulayıcı Donanımı Satın Alımı](https://www.youtube.com/watch?v=C2wwu1IlhDc) - _EthStaker 2022_
- [Adım Adım: Nephele 2.0 Testnet'e nasıl katılınır](https://kb.beaconcha.in/guides/tutorial-eth2-multiclient) - _Butta_
