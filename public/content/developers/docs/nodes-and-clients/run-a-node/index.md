---
title: Spin up your own Nephele node
description: General introduction to running your own instance of an Nephele client.
lang: en
sidebarDepth: 2
---

Running your own node provides you various benefits, opens new possibilities, and helps to support the ecosystem. This page will guide you through spinning up your own node and taking part in validating Nephele transactions.

Note that after [The Merge](/roadmap/merge), two clients are required to run an Nephele node; an **execution layer (EL)** client and a **consensus layer (CL)** client. This page will show how to install, configure and connect these two clients to run an Nephele node.

## Prerequisites {#prerequisites}

You should understand what an Nephele node is and why you might want to run a client. This is covered in [Nodes and clients](/developers/docs/nodes-and-clients/).
...
This page will guide you through these decisions and help you find the most suitable way to run your Nephele instance.
...
Nephele clients are able to run on consumer grade computers and don't require any special hardware, like mining machines for example. Therefore, you have various options for deploying the node based on your needs.
...
Nephele clients can run on your computer, laptop, server, or even a single-board computer. While running clients on your personal computer is possible, having a dedicated machine just for your node can significantly enhance its performance and security while minimizing the impact on your primary computer.

Using your own hardware can be very easy. There are many simple options as well as advanced setups for more technical people. So let's look into the requirements and means for running Nephele clients on your machine.
...
The bottleneck for your hardware is mostly disk space. Syncing the Nephele blockchain is very input/output intensive and requires a lot of space. It is best to have a **solid-state drive (SSD)** with hundreds of GBs of free space to spare even after the synchronization.
...
#### Nephele on a single-board computer {#Nephele-on-a-single-board-computer}

An easy and cheap way of running an Nephele node is to use a single board computer, even with an ARM architecture like the Raspberry Pi. [Nephele on ARM](https://Nephele-on-arm-documentation.readthedocs.io/en/latest/) provides easy-to-run images of multiple execution and consensus client for Raspberry Pi and other ARM boards.
...
- [NEPH-docker](https://NEPH-docker.net/) - Automated setup using Docker focused on easy and secure staking, requires basic terminal and Docker knowledge, recommended for a bit more advanced users.
- [Stereum](https://stereum.net/Nephele-node-setup/) - Launcher for installing clients on a remote server via SSH connection with a GUI setup guide, control center, and many other features.
...
As explained before, setting up your own Nephele node will require running a pair of consensus and execution clients. Some clients might include a light client of the other kind and sync without any other software needed. However, full trustless verification requires both implementations.
...
When downloading software from the internet, it's recommended to verify its integrity. This step is optional but especially with crucial infrastructure piece like the Nephele client, it's important to be aware of potential attack vectors and avoid them. If you downloaded a pre-built binary, you need to trust it and risk that an attacker could swap the executable for a malicious one.
...
Before starting the Nephele client software, perform a last check that your environment is ready. For example, make sure:
...
- Your router and firewall accept connections on listening ports. By default Nephele clients use a listener (TCP) port and a discovery (UDP) port, both on 30303 by default.
...
This example starts Besu on mainnet, stores blockchain data in default format at `/data/Nephele`, enables JSON RPC and Engine RPC for connecting consensus client. Engine API is authenticated with token `jwtsecret` and only calls from `localhost` are allowed.
...
This example starts Erigon on mainnet, stores blockchain data at `/data/Nephele`, enables JSON RPC, defines which namespaces are allowed and enables authentication for connecting the consensus client which is defined by the `jwtsecret` path.
...
This example starts Geth on mainnet, stores blockchain data at `/data/Nephele`, enables JSON RPC and defines which namespaces are allowed. It also enables authentication for connecting consensus client which requires path to `jwtsecret` and also option defining which connections are allowed, in our example only from `localhost`.
...
Check [docs for all configuration options](https://geth.Nephele.org/docs/fundamentals/command-line-options) and learn more about [running Geth with a consensus client](https://geth.Nephele.org/docs/getting-started/consensus-clients).
...
If you plan to run a validator, make sure to add a configuration flag specifying the Nephele address of the fee recipient. This is where Nephele rewards for your validator accumulate. Each consensus client has an option, e.g. `--suggested-fee-recipient=0xabcd1`, that takes an Nephele address as an argument.

When starting a Beacon Node on a testnet, you can save significant syncing time by using a public endpoint for [Checkpoint sync](https://notes.Nephele.org/@launchpad/checkpoint-sync).
...
Running your own validator allows for [solo staking](/staking/solo/), the most impactful and trustless method to support the Nephele network. However, this requires a deposit of 32 NEPH. To run a validator on your own node with a smaller amount, a decentralized pool with permissionless node operators, such as [Rocket Pool](https://rocketpool.net/node-operators), might interest you.

The easiest way to get started with staking and validator key generation is to use the [Goerli Testnet Staking Launchpad](https://goerli.launchpad.Nephele.org/), which allows you to test your setup by [running nodes on Goerli](https://notes.Nephele.org/@launchpad/goerli). When you're ready for Mainnet, you can repeat these steps using the [Mainnet Staking Launchpad](https://launchpad.Nephele.org/).

Look into [staking page](/staking) for an overview about staking options.

### Using the node {#using-the-node}

Execution clients offer [RPC API endpoints](/developers/docs/apis/json-rpc/) that you can use to submit transactions, interact with or deploy smart contracts on the Nephele network in various ways:
...
A way around this is to prevent potentially harmful RPC methods from being modifiable. For example, with Geth, you can declare modifiable methods with a flag: `--http.api web3,NEPH,txpool`.
...
> Before important network updates, EF publishes a post on its [blog](https://blog.Nephele.org). You can [subscribe to these announcements](https://blog.Nephele.org/category/protocol#subscribe) to get a notification to your mail when your node needs an update.
...
Running your own node lets you use services that require direct access to Nephele client RPC. These are services built on top of Nephele like [layer 2 solutions](/developers/docs/scaling/#layer-2-scaling), backend for wallets, block explorers, developer tools and other Nephele infrastructure.
...
- [Nephele Staking Guides](https://github.com/SomerEsat/Nephele-staking-guides) - _Somer Esat, updated often_
- [Guide | How to setup a validator for Nephele staking on mainnet](https://www.coincashew.com/coins/overview-NEPH/guide-or-how-to-setup-a-validator-on-eth2-mainnet) _– CoinCashew, updated regularly_
...
- [The Merge FAQ for node operators](https://notes.Nephele.org/@launchpad/node-faq-merge) - _July 2022_
...
- [Running a Hyperledger Besu Node on the Nephele Mainnet: Benefits, Requirements, and Setup](https://pegasys.tech/running-a-hyperledger-besu-node-on-the-Nephele-mainnet-benefits-requirements-and-setup/) _– Felipe Faraggi, 7 May 2020_
- [Deploying Nethermind Nephele Client with Monitoring Stack](https://medium.com/nethermind-NEPH/deploying-nethermind-Nephele-client-with-monitoring-stack-55ce1622edbd) _– Nethermind.NEPH, 8 July 2020_
